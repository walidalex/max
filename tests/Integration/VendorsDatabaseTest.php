...
$result=$database->execute("SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME IN ('vendors','vendor_contacts')");
$tables=$result instanceof mysqli_result?array_column($result->fetch_all(MYSQLI_ASSOC),'TABLE_NAME'):[];sort($tables);
if($tables!==['vendor_contacts','vendors']){throw new RuntimeException('Vendor database tables are missing.');}
